...
int main(int argc, char** argv) {
    ...
    float totnumMutationsSurvive = 0;
    float meanBurden = 0;
	...
	fprintf(outfile_handle, "meanburden\tnmutsurvive\n");		
	...
	for (num_trials=0; num_trials<num_desired; num_trials++) {
		...
		// calculate mean burden for this iteration
		for (i = 0; i < numFounder*2*numFounderMutations; i++) {
		    if (binHeights[i] > 0) {
                meanBurden += i*(1.0*binHeights[i])/totalsubjCounted;
	        }
		}
		
		// calculate number of surviving mutations
		for (i = 0; i < numFounder*numFounderMutations; i++) {
            if (numMutationsSurvive[i] > 0) {
                totnumMutationsSurvive++;
            }
    	}    	
    	
        fprintf(outfile_handle, "%f\t%f\n", meanBurden, totnumMutationsSurvive);
